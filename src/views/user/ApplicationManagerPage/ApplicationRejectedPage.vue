<template>
  <div class="container">
    <h3 class="p-3 text-center">Document Rejected Management</h3>
...
export default {
  ...
  methods: {
    ...
    goToNextPage(data) {
      ...
      this.$router.push('/application-view');
  ...
  computed: {
    ...
    filteredApplicationByStatus() {
      let filteredArray = [];
      for (let i = 0; i < this.applications.length; i++) {
        if (this.applications[i].user_receiver[0].status === 2 && this.applications[i].user_receiver[0].delete === false) {
          filteredArray.push(this.applications[i]);
        }
      }
  ...
  created() {
    ApiService.getApplication()
      .then((res) => {
        // Filter applications where receiver_id = user_id and status = 2
