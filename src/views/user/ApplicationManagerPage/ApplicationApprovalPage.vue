<template>
  <div class="container">
    <h3 class="p-3 text-center">Document Approval</h3>
...
export default {
        ...
        .then(() => {
          ...
          ApiService.getApplication()
            .then((res) => {
              ...
              this.applications = this.applications.filter((application) => {
                return application.sender_id === this.userId && application.status === 2 && application.delete_by_sender === false;
              });
            })
            .catch((err) => {
              console.log(err);
  ...
  created() {
    this.applications = ApiService.getApplication()
      .then((res) => {
        this.applications = res.data;
        // get application which sender is current user
        this.applications = this.applications.filter((application) => {
